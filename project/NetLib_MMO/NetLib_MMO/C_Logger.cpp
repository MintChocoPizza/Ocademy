...
static en_LOG_LEVEL g_LogLevel = LEVEL_ERROR;

#define CONSOLE 3
#define TEXTFILE 4

WCHAR* g_pszGlobalFolerPath;
...
void NTAPI tls_callback(PVOID DllHandle, DWORD dwReason, PVOID)
{
	...
	{
		if (g_pszFolerPath == nullptr && g_pLogBufForFileConsole == nullptr)
		{
			allocateMemory();

			if (g_pszGlobalFolerPath != nullptr && g_pszFolerPath != nullptr)
			{
				StringCchPrintf(g_pszFolerPath, MAX_PATH, L"%s", g_pszGlobalFolerPath);
			}
		}
	}
	else if (dwReason == DLL_THREAD_DETACH || dwReason == DLL_PROCESS_DETACH)
	{
		if (g_pszFolerPath != nullptr && g_pLogBufForFileConsole != nullptr)
		{
			freeMemory();
...
void InitLogger(const WCHAR* szLogPath)
{
	...
	if (g_pszGlobalFolerPath == nullptr)
	{
		InitializeSRWLock(&g_srwForFILEIO);
		g_pszGlobalFolerPath = (WCHAR*)HeapAlloc(GetProcessHeap(), HEAP_GENERATE_EXCEPTIONS, MAX_PATH * sizeof(WCHAR));
		SYSLOG_DIRECTORY(szLogPath);
		StringCchPrintf(g_pszGlobalFolerPath, MAX_PATH, L"%s", g_pszFolerPath);
	}
	else
	{
		__debugbreak();
